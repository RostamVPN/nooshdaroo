...
use crate::dns::{self, DnsMessage, DnsName, DnsRR, encode_rdata_txt};
...
async fn send_one_response(
    ...
    if resp.rcode() == dns::RCODE_NO_ERROR && resp.question.len() == 1 {
        ...
        resp.answer[0].data = encode_rdata_txt(&payload);
    }
...
async fn handle_tcp_conn(
    ...
    loop {
        ...
        if final_resp.rcode() == dns::RCODE_NO_ERROR && final_resp.question.len() == 1 {
            ...
            final_resp.answer[0].data = encode_rdata_txt(&resp_payload);
        }
