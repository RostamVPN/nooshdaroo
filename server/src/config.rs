...
pub struct ServerConfig {
    ...
    /// Enable LiteWeb content proxy for HTTP (port 80) requests.
    /// When enabled, HTTP requests through SOCKS5 are intercepted: the page is
    /// fetched server-side, readable content is extracted, images are optimized,
    /// and a minimal HTML page is returned. HTTPS and all non-HTTP traffic is
    /// always passed through as raw SOCKS5 relay unchanged.
    #[serde(default)]
    pub liteweb_enabled: bool,
