...
struct Args {
    /// Listen addresses for incoming DNS queries (can specify multiple for multi-homed hosts)
    #[arg(short, long, default_value = "0.0.0.0:53", num_args = 1..)]
    listen: Vec<String>,
...
impl TxidRewriter {
    ...
    /// Returns (client_addr, listen_socket_idx) so response goes from the correct source IP.
    fn rewrite_response(
        &self,
        packet: &mut [u8],
        backend_idx: usize,
        ...
        Some((pq.client_addr, pq.listen_socket_idx))
    }
...
async fn backend_recv_loop(
    backend_idx: usize,
    socket: Arc<UdpSocket>,
    listen_sockets: Arc<Vec<Arc<UdpSocket>>>,
    ...
    loop {
        ...
        if let Some((client_addr, sock_idx)) = txid_rewriter.rewrite_response(&mut packet, backend_idx) {
            // Send response from the same socket that received the query
            // (preserves source IP on multi-homed hosts)
            let sock = &listen_sockets[sock_idx % listen_sockets.len()];
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Bind listen sockets — one per address for correct source IP on multi-homed hosts
    let mut listen_sockets = Vec::new();
    for addr_str in &args.listen {
        let addr: SocketAddr = addr_str.parse().map_err(|e| {
            format!("invalid listen address '{}': {}", addr_str, e)
        })?;
        let sock = bind_listen_socket(&addr, args.reuseport).await?;
        info!("listening on {}", addr);
        listen_sockets.push(Arc::new(sock));
    }
    let listen_sockets = Arc::new(listen_sockets);
    ...
    // Spawn per-backend receive loops (responses route back through correct listen socket)
    for (i, backend) in backends.iter().enumerate() {
        tokio::spawn(backend_recv_loop(
            i,
            Arc::clone(&backend.socket),
            Arc::clone(&listen_sockets),
    ...
    info!(
        "dnstt-lb ready: {} listen socket(s), {} backends, health={}s, stats={}s",
        listen_sockets.len(),
        backends.len(),
    ...
    // Spawn a receive loop per listen socket — each socket handles one IP
    // and records its index so responses go from the same IP.
    for (sock_idx, sock) in listen_sockets.iter().enumerate() {
        let sock = Arc::clone(sock);
        let backends = Arc::clone(&backends);
        let txid_rewriter = Arc::clone(&txid_rewriter);
        let stats = Arc::clone(&stats);
        let domain_suffix_clone = domain_suffix.clone();

        tokio::spawn(async move {
            let domain_ref = domain_suffix_clone.as_deref();
            let fallback_id: [u8; 8] = [0xff; 8];
            let mut buf = [0u8; 4096];
            ...
            loop {
                let (n, client_addr) = match sock.recv_from(&mut buf).await {
                    Ok(v) => v,
                    Err(e) => {
                        debug!("recv_from error on socket {}: {}", sock_idx, e);
                        continue;
                    }
                };
                ...
                if txid_rewriter
                    .rewrite_forward(&mut packet, client_addr, backend_idx, sock_idx)
                    .is_none()
                {
                    continue;
                }
