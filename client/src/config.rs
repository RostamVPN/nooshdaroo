...
pub async fn fetch_ota_config(
    ...
    for resolver_str in OTA_RESOLVERS {
        ...
        // Try TXT first (highest capacity), then A records (censor-resistant).
        // In Iran (Mar 2026), TXT is blocked — A records pass through.
        let txt_records = match fetch_txt_records(ota_domain, resolver).await {
            Ok(recs) if !recs.is_empty() => recs,
            _ => {
                // TXT failed or empty — try A records
                match fetch_a_records(ota_domain, resolver).await {
                    Ok(recs) if !recs.is_empty() => recs,
                    _ => {
                        log::debug!("OTA: no TXT or A records from {}", resolver_str);
                        continue;
                    }
                }
            }
...
/// Send a raw DNS A query and reassemble the response into OTA TXT-compatible strings.
///
/// Each A record = 4 bytes of payload. The server publishes OTA config as A records
/// where each "IP address" is actually 4 bytes of data. We concatenate all A records,
/// strip the 2-byte length header, and decode the payload as if it were TXT record data.
///
/// This allows OTA config to survive censor environments that block TXT records.
async fn fetch_a_records(domain: &str, resolver: SocketAddr) -> Result<Vec<String>, String> {
    let sock = UdpSocket::bind("0.0.0.0:0").await
        .map_err(|e| format!("UDP bind: {}", e))?;
    ...
    parse_a_response_as_txt(&resp[..len])
}

/// Parse A records from a DNS response and reconstruct as TXT-compatible strings.
/// Each A record is 4 bytes. Concatenate all, strip 2-byte length header, then
/// split the raw data into strings as if they were TXT character-strings.
fn parse_a_response_as_txt(wire: &[u8]) -> Result<Vec<String>, String> {
    ...
    // Collect all A record RDATA (4 bytes each)
    ...
    for _ in 0..ancount {
        ...
        if rtype == 1 && rdlength == 4 {
            // A record: 4 bytes of payload
            raw_bytes.extend_from_slice(&wire[rdata_start..rdata_end]);
        }
        pos = rdata_end;
    }
    ...
    // First 2 bytes = total payload length (strip padding from last A record)
    let total_len = u16::from_be_bytes([raw_bytes[0], raw_bytes[1]]) as usize;
    let payload = if total_len + 2 <= raw_bytes.len() {
        &raw_bytes[2..2 + total_len]
    } else {
        &raw_bytes[2..]
    };

    // Decode as UTF-8 text — OTA config is encoded the same way regardless of transport
    if let Ok(s) = std::str::from_utf8(payload) {
        // Split into lines/records (same format as TXT records)
        Ok(s.lines().map(|l| l.to_string()).collect())
    } else {
        Err("A record payload is not valid UTF-8".into())
    }
}
